...
optuna.logging.set_verbosity(optuna.logging.WARNING)  # donot see the warning infomation


def feature_classification_process_auto(classifier_tags, train_frame, validation_frame, save_dir, class_weight,
                                        resampled=True,
                                        resampled_method='SMOTE',
                                        random_state=42,
                                        scoring='accuracy'):
    """
    ...
    :param save_dir:
    :param class_weight: bool, will  be used in classifier
    :param resampled: bool, if True, resample strategy will be used
    :param resampled_method: resampled method name such as SMOTE, undersampl etc al.
    :param random_state:
    :param scoring: to evaluate the classifier
    :return:
    """
    classifier_dict = {
        0: 'LogsticRegression',
    ...
    for classifier_tag in classifier_tags:  # for each classifier
        classification_method = classifier_dict[classifier_tag]
        columns.append(classification_method)
        file_name = os.path.join(method_save_dir, classification_method + '_best.pkl') #the trained model
        if os.path.exists(file_name): #if the model is trained,load it
            with open(file_name, 'rb') as f:
                clf_best = pickle.load(f)
                print('load trained model:', classification_method)
        else:

            trial_dir = os.path.join(method_save_dir, 'trial_history')#to save the trial model to get the best classifier
            os.makedirs(trial_dir)
            if classifier_tag == 0:  # logistic regression
                # if os.path.exists(os.path.join(method_save_dir,'{}_trained.'))
                def objective_here(trial):#the objective settings
                    params = {
                    ...
                    model = LogisticRegression(**params, random_state=random_state, class_weight=class_weight)
                    model.fit(feature_train, label_train)
                    with open(os.path.join(trial_dir, "{}.pickle".format(trial.number)), "wb") as fout:
                        pickle.dump(model, fout)
                    pred = model.predict_proba(feature_validation)[:, 1]
                    auc_value = roc_auc_score(label_validation, pred)
                    return auc_value
                study = optuna.create_study(direction='maximize', study_name='LogisticRegression',
                                            pruner=optuna.pruners.HyperbandPruner())
                study.optimize(objective_here, n_trials=300)
                print('numbers of the finished trials:', len(study.trials))
                print('the best params:', study.best_trial.params)
                print('the best value:', study.best_value)
                # with open(os.path.join(method_save_dir, "{}.pickle".format(study.best_trial.number)), "rb") as fin:
                #     best_clf = pickle.load(fin)
                shutil.copy(os.path.join(trial_dir, "{}.pickle".format(study.best_trial.number)),
                            os.path.join(method_save_dir, '{}_best.pickle'.format(classification_method)))
            ...
            elif classifier_tag == 10:  # xgboost

            file_name = os.path.join(method_save_dir, classification_method + '_trained.pickle')
